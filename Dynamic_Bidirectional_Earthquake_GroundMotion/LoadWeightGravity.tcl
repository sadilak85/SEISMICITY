...
for {set i 1} {$i <= [lindex $NStory $numInFile 0]} {incr i 1} {
	...
	lappend iFloorWeight $iFloorWeighttmp;   # Weight of each floor Frames for each building
...
for {set i 0} {$i <=[expr [lindex $NStory $numInFile 0]-1]} {incr i 1} { ;	
	set FloorWeight [lindex $iFloorWeight $numInFile $i];
	lset iFj $numInFile $i [expr $FloorWeight*[lindex $FloorHeight $numInFile $i]/[lindex $sumWiHi $numInFile 1]*[lindex $WeightTotal $numInFile 1]];		
}
