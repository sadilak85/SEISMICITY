...
set outputFilename "INPUT_"
set outputFileExt ".tcl"
set Buildingnum 1
set ainputFilename "INPUT_1.tcl"


set outputfile [open $outputFilename$Buildingnum$outputFileExt w]
         ...
         if {[llength $line] == 0} {
			...
			foreach word [split $line] {
				if {[string match $word "#BUILDING_ID"] == 1} {
					...
					lappend ainputFilename $outputFilename$Buildingnum$outputFileExt
					set outputfile [open $outputFilename$Buildingnum$outputFileExt w]
					break
...
set NBay 1;	#min number of Bays in X direction
set NBayZ 1;
...
for {set numInFile 0} {$numInFile <= [expr $Buildingnum-1]} {incr numInFile 1} {
	...
	set flag 0
	puts $inFileID
	puts [lindex $ainputFilename $numInFile 0]
		foreach line [split [read $inFileID] \n] {
			puts $inFileID
			if {[llength $line] == 0} {
				# Blank line --> do nothing
				...
				if {$flag == 1} {
					...
					foreach word [split $list] {
						if {$checkID < [string index $word 0]} {
							set NStory [expr [string index $word 0]-1];			# number of stories above ground level  # Assuming right inputting
							break
						} else {
							break
						}
...
set aFloorWeight {}
for {set i 1} {$i <= [expr $NStory+1]} {incr i 1} {
	lappend aFloorWeight 0
}

if [catch {open [lindex $ainputFilename $Buildingnum 0] r} inFileID] {
			...
			if {$flag == 1} {
				...
				foreach word [split $list] {
					...
					if {[string index $word 0] ne "1"} {
						...
						lset aFloorWeight 0 $inhalt [expr {[lindex $aFloorWeight 0 $inhalt] + $WeightNode}];   #burda row 0 yerine variable olcak 2d de   
					}
					break
				}
			}
