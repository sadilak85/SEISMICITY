...
set outputfile [open "nodeout.txt" w]
for {set frame 1} {$frame <=[expr $NFrame]} {incr frame 1} {
	...
	for {set level 1} {$level <=[expr $NStory+1]} {incr level 1} {
		...
		for {set pier 1} {$pier <= [expr $NBay+1]} {incr pier 1} {
			...
			puts $outputfile $nodeID\t$X\t$Y\t$Z
		}
	}
}

set outputfile2 [open "diaphragmout.txt" w]
...
for {set level 2} {$level <=[expr $NStory+1]} {incr level 1} {
	...
	puts $outputfile $MasterNodeID\t$Xa\t$Y\t$Za
	...
	for {set frame 1} {$frame <=[expr $NFrame]} {incr frame 1} {
		for {set pier 1} {$pier <= [expr $NBay+1]} {incr pier 1} {
			...
			puts $outputfile2 $MasterNodeID\t$nodeID
		}
	}
}
close $outputfile
...
set level 0
set outputfile [open "eleout.txt" w]
puts $outputfile columns
for {set frame 1} {$frame <=[expr $NFrame]} {incr frame 1} {
	for {set level 1} {$level <=$NStory} {incr level 1} {
		for {set pier 1} {$pier <= [expr $NBay+1]} {incr pier 1} {
			...
			puts $outputfile $elemID\t$nodeI\t$nodeJ
		}
	}
